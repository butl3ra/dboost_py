...
# --- socp constraints:
n_samples = 10
L = np.random.normal(size=(n_samples,n_z))
Q = np.dot(L.T, L)
Q = Q / n_samples
...
# --- socp only
#A = np.concatenate((A_0,np.zeros((1,n_z)),Q12))
#b = np.concatenate((b_0,b_socp,np.zeros((n_z,1))))
...
control_gb = grad_boost_control(num_trees=100, verbose=True, weight_tol=0.01,
                                loss_tol=10 ** -5, alpha_min=0, alpha_max=1000)
